...
van Zundert, C, and Hoijtink, H. (2019). Teacher's 
corner: Evaluating informative hypotheses using the
Bayes factor in structural equation models. DOI: PsyArchiv
...
`growth` functions.
4) A named vector containing the estimates resulting from a statistical analysis. Note that, named means that each estimate has to be nameled such that it can be referred to in hypotheses.
...
implemented in `lavaan`. Note that, by default, 
...
available.
2) Specify a model using the `model <- ...` command.
...
number to create a repeatable random number sequence.
5) `results <- bain(x,hypotheses,fraction = 1,standardize = TRUE)`.
...
* When using `bain` with a `lm` or `t_test` or `lavaan` object, (unique abbreviations of) the names
...
analyses are executed). If, for example, the names are `cat` and `dog`, `c`
...
functions of the predictors (for example, squared
predictors)
can not be processed by `bain`.** However, note that interactions 
between predictors can
be processed. Note furthermore, that one can compute a new variable
(for example, a squared predictor) and add this variable to the 
model specification of `lm`.
...
c) Bayesian multiple group latent regression
...
See van Lissa et al. (2019) for further elaborations
...
set.seed(100)
y <- bain(fit1,hypotheses1,fraction=1,standardized=TRUE)
...
parest <- parameterEstimates(fit3, standardized = TRUE)
...
a named vector object. See van Lissa et al. (2019) for further elaborations
