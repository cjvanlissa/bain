...
If in a `lavaan` model specification parameters are labelled, e.g., as in 
...
analyses are executed and/or parameters are labelled). If, 
for example, the names are `cat` and `dog`, `c`
and `d ` would be unique abreviations. If, for example, the names are `cato` 
and `cata` the whole
...
labelled: "Bayesian informative hypothesis testing for an object 
of class bain_htest" (which denotes that a `t-test` was executed).
...
labelled: "Bayesian informative hypothesis testing for an object of 
...
If a `lavaan` object is used, the main output table is labelled: 
...
If a `named vector` object is used, the main output table is labelled: 
"Bayesian informative hypothesis testing for an object 
of class named numeric" (which denotes that `named vector` input was used).
