...
#' in \code{hypotheses}.
#' }
#' @param hypothesis	A character string containing the informative hypotheses to evaluate (see Details).
...
#' that can be processed by \code{bain}. Note that, \code{lm} and \code{t.test} will apply list-wise deletion
#' if there are cases with missing values in the variables used.
#' \item Displays the estimates such that in the next step new names can be given to the  estimates.
#' These names are used to specify \code{hypotheses}. For \code{lm()}, \code{coef(x)} will display the
...
#' An example is \code{labels <- c("a", "b", "c")}.
#' \item \link{set.seed}\code{(seed)}. Set \code{seed} equal to an integer number to create a repeatable
#' random number sequence.
#' \item \code{results <- bain(x,hypotheses)} or \code{results <- bain(x,hypotheses,standardize = TRUE)}. The first call to \code{bain}
...
#' \item \code{descriptives(results, ci=0.95)} Present descriptives for the parameters used to
...
#' estimates of the parameters used to specify \code{hypotheses}; 2) a list containing the covariance matrix of these parameters; and, 3)
...
#' specify \code{hypotheses} possibly augmented with the estimates of other parameters; a list containing per group the covariance
...
#' "numbers", ".", and "_". These labels are used to specify \code{hypotheses} (see below). An example is
#' \code{labels <- c("a", "b", "c")}.
#' \item \link{set.seed}\code{(seed)}. Set \code{seed} equal to an integer number to create a repeatable
#' random number sequence.
#' \item \code{results <- bain(estimates,hypotheses,n=.,Sigma=.,group_parameters=2,joint_parameters=0)} executes \code{bain} with
...
#' \item \code{Sigma} A list of covariance matrices. In case of one group analyses the list contains one
#' covariance matrix. In case of multiple group analyses the list contains one covariance matrix for each group.
#' See the Example section and Hoijtink, Gu, and Mulder (2018) for further instructions.
#' \item \code{group_parameters} The number of group specific parameters.  In, for example, an ANOVA with three groups,
#' \code{estimates} will contain three sample means, \code{group_parameters = 1}
#' because each group is characterized by one mean, and \code{joint_parameters = 0} because there are no parameters
#' that apply to each of the groups.. In, for example, an ANCOVA with  three groups and
...
#' and two groups (a between factor with two levels and a  within factor with four levels) \code{estimates} will contain
...
#' \item \code{joint_parameters} In case of one group \code{joint_parameters = 0}.
...
#' specifies two hypotheses using three estimates with names "a", "b", and "c", respectively.
#'
...
#' \item Linear combinations of parameters must be specified adhering to the following rules:
#'         \enumerate{ \item Each parameter name is used at most once.
#'                     \item Each parameter name may or may not be pre-multiplied with a number.
#'                     \item A constant may be added or subtracted from each parameter name.
#'                     \item A linear combination can also be a single number.}
#' Examples are: \code{3 * a + 5}; \code{a + 2 * b + 3 * c - 2}; \code{a - b}; and \code{5}.
...
#' message or render an output table containing \code{Inf}'s.
#'
#' @return The commands \code{bain()} or \code{results<-bain()} followed by \code{results} or \code{print(results)} will
...
#' Additionally, using \code{descriptives(results, ci=0.95)}, a descriptives matrix can be obtained
...
#' }
#'
#' @author The main authors of the bain package are Xin Gu (computational core of the code, statistical underpinings),
#' Caspar van Lissa (creation of CRAN package, user interface and maintainer), Herbert Hoijtink (statistical underpinnings) and Joris Mulder
...
#'
#'
#' @references All references are retrievable via or from \url{https://informative-hypotheses.sites.uu.nl/software/bain/}
#'
...
#'
#' @seealso The bain website at \url{https://informative-hypotheses.sites.uu.nl/software/bain/} contains downloadable legacy
...
#'
#' \dontrun{
#' # Note that, each of the examples given below can be run by copy pasting them into the
#' # Source screen of RStudio. An Examples.R file can also be downloaded from the baij website at
#' # \url{https://informative-hypotheses.sites.uu.nl/software/bain/}
#'
#' # ==============================================================
...
#' # Note that, ONLY the type of calls to LM presented in the examples can be processed by bain. Calls structured in a
#' # different manner will either render an error message, or results that are meaningless.
#' # + Bayesian ANOVA. The example concerns a one-way ANOVA. Two-way or higher
#' #   order ANOVA's can only be handled by recoding all factors into one factor. If, for example,
#' #   there is a factor sex with levels man and woman, and a factor age with levels young and old, these
#' #   have to be recoded in a new factor sexage with levels manyoung, manold, womanyoung, womanold
#' # + Bayesian ANCOVA. The example concerns a one-way ANCOVA. Two-way or higher
#' #   order ANCOVA's can only be handled by recoding all factors into one factor.
...
#' # inspect the estimates of the group means
#' print(ttest)
#' # compute the pooled within standard deviation using the variance of x (ttest$v[1]) and y (ttest$v[2])
#' pwsd <- sqrt(((length(x) -1) * ttest$v[1] + (length(y)-1) * ttest$v[2])/((length(x) -1) + (length(y) -1)))
#' # print pwsd in order to be able to include it in the hypothesis. Its value is 12.60
#' print(pwsd)
#' # assign names to the estimates
#' ttest <- label_estimates(ttest, c("boy","girl"))
#' # set a seed value
#' set.seed(100)
#' # test hypotheses (the means of boy and girl differ less than .2 * pwsd = 2.52 VERUS
#' # the means differ more than .2 * pwsd = 2.52) with bain
#' # note that, .2 is a value for Cohen's d reflecting a "small" effect, that is, the means
#' # differ less or more than .2 pwsd.
...
#' # make a factor of variable site
#' sesamesim$site <- as.factor(sesamesim$site)
#' # Center the covariate. If centered the coef() command below displays the adjusted means.
...
#' # set a seed value
#' set.seed(100)
#' # test hypotheses with bain. Note that standardized = FALSE denotes that the hypotheses
#' # are in terms of unstandardized regression coefficients
#' results<-bain(regr, "age = 0 & peab=0 & pre=0 ; age > 0 & peab > 0 & pre > 0 ", standardize = FALSE)
#' # display the results
#' print(results)
#' # obtain the descriptives table
#' descriptives(results, ci = 0.95)
#' # Since it is only meaningful to compare regression coefficients if they are measured
#' # on the same scale, bain can also evaluate standardized regression coefficients
#' # (based on the seBeta function by Jeff Jones and Niels Waller):
#' # set a seed value
#' set.seed(100)
#' # test hypotheses with bain. Note that standardized = TRUE denotes that the hypotheses
#' # are in terms of standardized regression coefficients
...
#' # for the ANOVA this is simply a list containing for each group the variance of the mean
#' # note that, the within group variance as estimated using lm is used to compute the
#' # variance of each of the means! See, Hoijtink, Gu, and Mulder (2018) for further elaborations.
...
#' # set a seed value
#' set.seed(100)
#' # test hypotheses with bain. Note that there are multiple groups characterized by one mean,
#' # therefore group_parameters=0. Note that are no joint parameters, therefore, joint_parameters=0.
#' results <- bain(estimate,"site1=site2=site3=site4=site5; site2>site5>site1>site3>site4",n=ngroup,Sigma=covlist,group_parameters=1,joint_parameters = 0)
#' # display the results
#' print(results)
#' # obtain the descriptives table
#' descriptives(results, ci = 0.95)
...
#' coef(ancov2)
#' # collect the estimates of the adjusted means and regression coefficient of the covariate in a vector
...
#' ngroup <- table(sesamesim$site)
#' # compute for each group the covariance matrix of the parameters of that group and collect these in a list
#' # note that, the residual variance as estimated using lm is used to compute these covariance matrices
#' var <- (summary(ancov2)$sigma)**2
#' # below, for each group, the covariance matrix of the adjusted mean and covariate is computed
...
#' # set a seed value
#' set.seed(100)
#' # test hypotheses with bain. Note that there are multiple groups characterized by one adjusted mean,
#' # therefore group_parameters=0. Note that there is one covariate, therefore, joint_parameters=1.
#' results2<-bain(estimates,"v.1=v.2=v.3=v.4=v.5;v.2 > v.5 > v.1 > v.3 >v.4;",n=ngroup,Sigma=covariances,group_parameters=1,joint_parameters = 1)
#' # display the results
#' print(results2)
#' # obtain the descriptives table
...
#' # take a look at the estimated means and their names
#' coef(within)
#' # note that the model specified in lm has three dependent variables. Consequently,
#' # the estimates rendered by lm are collected in a "matrix". Since bain needs a named
#' # "vector" containing the estimated means, the [1:3] code is used to select the three
#' # means from a matrix and store them in a vector.
...
#' # set a seed value
#' set.seed(100)
#' # test hypotheses with bain. Note that there is one group, therefore group_parameters=0.
#' # Note that there are three means, therefore, joint_parameters=3.
#' results <- bain(estimate,"pre = post = fu; pre < post < fu", n=ngroup, Sigma=covmatr, group_parameters=3, joint_parameters = 0)
#' # display the results
#' print(results)
#' # obtain the descriptives table
#' descriptives(results, ci = 0.95)
...
#' cov1 <- c(vcov(bw)[1,1],vcov(bw)[1,3],vcov(bw)[1,5],vcov(bw)[3,1],vcov(bw)[3,3],vcov(bw)[3,5],vcov(bw)[5,1],vcov(bw)[5,3],vcov(bw)[5,5])
#' cov1 <- matrix(cov1,3,3)
#' cov2 <- c(vcov(bw)[2,2],vcov(bw)[2,4],vcov(bw)[2,6],vcov(bw)[4,2],vcov(bw)[4,4],vcov(bw)[4,6],vcov(bw)[6,2],vcov(bw)[6,4],vcov(bw)[6,6])
#' cov2 <- matrix(cov2,3,3)
#' covariance<-list(cov1,cov2)
#' # set a seed value
#' set.seed(100)
#' # test hypotheses with bain. Note that there are multiple groups characterized by three means,
#' # therefore group_parameters=3. Note that there are no additional parameters, therefore, joint_parameters=0.
#' results <-bain(estimate, "pre1 - pre2 = post1 - post2 = fu1 -fu2; pre1 - pre2 > post1 - post2 > fu1 -fu2"  , n=ngroup, Sigma=covariance, group_parameters=3, joint_parameters = 0)
#' # display the results
#' print(results)
#' # obtain the descriptives table
#' descriptives(results, ci = 0.95)
...
#' sesamesim$peabody <- (sesamesim$peabody - mean(sesamesim$peabody))/sd(sesamesim$peabody)
#' sesamesim$prenumb <- (sesamesim$prenumb - mean(sesamesim$prenumb))/sd(sesamesim$prenumb)
#' # estimate the logistic regression coefficients
#' logreg <- glm(viewenc ~ age + peabody + prenumb, family=binomial, data=sesamesim)
...
#' set.seed(100)
#' # test hypotheses with bain. Note that there is one group, therefore group_parameters=0.
#' # Note that there are one intercept and three regression coefficients, therefore, joint_parameters=4.
#' results <- bain(estimate, "age = peab = pre; age > pre > peab", n=ngroup, Sigma=covmatr, group_parameters=4, joint_parameters = 0)
#' # display the results
#' print(results)
#' # obtain the descriptives table
#' descriptives(results, ci = 0.95)
...
#' # The research question concerns the probability of encouragement to view sesame street
#' # (a dichotomous 0/1 meaning no/yes dependent variable) for boys (sex=1) and girls (sex=2)
#' # adjusted for their age. This is a kind of ANCOVA with a dichotomous instead of a
#' # continuous dependent variable.
#' # ===============================================================================
#' # load the bain package which includes the simulated sesamesim.txt data set
#' library(bain)
#' # load the numDeriv package which will be used to compute the covariance matrix
#' # of the adjusted group coefficients and regression coefficient of age for the boys and the girls
...
#' # take a look at the estimates and their names
#' coef(logreg)
#' # collect the estimates obtained using the data of the boys AND the girls in a vector
#' estimates <-coef(anal)
#' # give names to the estimates
#' names(estimates) <- c("boys", "girls", "age")
#' # use numDeriv to compute the Hessian matrix and subsequently the covariance matrix for
#' # each of the two (boys and girls) groups
#' # The vector f should contain the regression coefficient of the group at hand and the
#' # regression coefficientof the covariate
#' #
#' # the first group
#' data <- subset(cbind(sesamesim$sex,sesamesim$age,sesamesim$viewenc), sesamesim$sex==1)
#' f <- 1
#' f[1] <- estimates[1] # the regression coefficient of boys
#' f[2] <- estimates[3] # the regression coefficient of age
#' #
#' # within the for loop below the log likelihood of the logistic regression model is computed
#' # using the data for the boys
#' logist1 <- function(x){
#'   out <- 0
#'   for (i in 1:ngroup[1]){
#'     out <- out + data[i,3]*(x[1] + x[2]*data[i,2]) - log (1 + exp(x[1] + x[2]*data[i,2]))
#'   }
#'   return(out)
#' }
#' hes1 <- hessian(func=logist1, x=f)
#' # multiply with -1 and invert to obtain the covariance matrix for the first group
#' cov1 <- -1 * solve(hes1)
#' #
#' # the second group
#' data <- subset(cbind(sesamesim$sex,sesamesim$age,sesamesim$viewenc), sesamesim$sex==2)
...
#' logist2 <- function(x){
#'   out <- 0
#'   for (i in 1:ngroup[2]){     # samp[2] is the sample size of the second group
#'     out <- out + data[i,3]*(x[1] + x[2]*data[i,2]) - log (1 + exp(x[1] + x[2]*data[i,2]))
#'   }
#'   return(out)
#' }
...
#' # set a seed value
#' set.seed(100)
#' # test hypotheses with bain. Note that there are multiple groups characterized by one adjusted group coefficient,
#' # therefore group_parameters=1. Note that there is one covariate, therefore, joint_parameters=1.
#' results <- bain(estimates, "boys < girls & age > 0", n=ngroup, Sigma=covariance, group_parameters=1, joint_parameters = 1)
#' # display the results
#' print(results)
#' # obtain the descriptives table
#' descriptives(results, ci = 0.95)
...
#' # An example of the Bayesian robust ANOVA with unequal within group variances. Robust
...
#' # Square the standard errors to obtain the variances of the sample trimmed means
#' var <- se^2
#' # Store the variances in a list of matrices
#' covlist <- list(matrix(var[1]),matrix(var[2]), matrix(var[3]),matrix(var[4]), matrix(var[5]))
#' # set a seed value
#' set.seed(100)
#' # test hypotheses with bain
#' results <- bain(estimates,"s1=s2=s3=s4=s5; s2>s5>s1>s3>s4",n=ngroup,Sigma=covlist,group_parameters=1,joint_parameters = 0)
#' # display the results
#' print(results)
#' # obtain the descriptives table
#' descriptives(results, ci = 0.95)
...
#' # inspect the help file to obtain further information about
#' # mice - also surf to http://www.stefvanbuuren.nl/mi/MICE.html to obtain further information and
#' # references for mice.
#' library(mice)
#' library(psych)
#' library(MASS)
...
#' #
#' for (i in 1:240){
#'   pmis1[i] <- exp(-30+sesamesim$prenumb[i]+sesamesim$postnumb[i])/(1+exp(-30+sesamesim$prenumb[i]+sesamesim$postnumb[i]))
...
#' # use mice to create 1000 imputed data matrices. Note that, the approach used below is only one manner in which
...
#' # of only the regression coefficients of funumb on prenumb and postnumband and the average of the 1000 covariance matrices.
#' # See Hoijtink, Gu, Mulder, and Rosseel (in press) for an explanation of the latter.
#' for(i in 1:M) {
...
#' # An example of the Bayesian structural equation modelling. For this topic additional
#' # examples are elaborated in Gu, Hoijtink, Mulder, and Rosseel (unpublished).
#' # ===============================================================================
#' # load the bain package
#' library(bain)
#' # load the lavaan package - look at the help file to obtain further information about lavaan - also surf to
#' # lavaan.ugent.be to obtain further information and references for lavaan - this example uses the
...
#' # determine the sample size
#' ngroup<-nobs(fit)
#' # collect the "standardized" estimates of the latent regression coefficients in a vector
#' # typing standardizedSolution(fit) in the console pane shows that the estimates can be
...
#' # typing lavInspect(fit, "vcov.std.all") in the console pane shows that the estimates can be
#' # found in the rows 10 and 11 crossed with columns 10 and 11
#' covariance<-list(lavInspect(fit, "vcov.std.all")[10:11,10:11])
#' # set a seed value
#' set.seed(100)
#' # test hypotheses with bain
#' results <- bain(estimate,"visual=textual=0; visual > textual > 0",n=ngroup,Sigma=covariance,group_parameters=2,joint_parameters = 0)
#' # display the results
#' print(results)
#' # obtain the descriptives table
#' descriptives(results, ci = 0.95)
